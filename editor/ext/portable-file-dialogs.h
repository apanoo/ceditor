    ...
    public:
      proc(dll const &lib, std::string const &sym)
          : m_proc(reinterpret_cast<T *>(
                ::GetProcAddress(lib.handle, sym.c_str()))) {}
...
inline bool &settings::flags(flag in_flag) {
  return const_cast<bool &>(
      static_cast<const settings *>(this)->flags(in_flag));
}
...
inline bool internal::dialog::kill() { return m_async->kill(); }
...
inline std::vector<std::string> internal::dialog::desktop_helper() const {
  ...
  return {flags(flag::has_zenity)
              ...
              : flags(flag::has_matedialog)
                    ...
                    : flags(flag::has_qarma)
                          ? "qarma"
                          : flags(flag::has_kdialog) ? "kdialog" : "echo"};
#endif
}
...
inline internal::file_dialog::file_dialog(
  ...
  m_async->start_func([this, in_type, title, default_path, filter_list,
    ...
    if (in_type == type::folder) {
      ...
      auto status = dll::proc<HRESULT WINAPI(LPVOID, DWORD)>(
          ole32, "CoInitializeEx")(nullptr, COINIT_APARTMENTTHREADED);
...
inline message::message(std::string const &title, std::string const &text,
  ...
  m_async->start_func([this, text, title,
                       style](int *exit_code) -> std::string {
    auto wtext = internal::str2wstr(text);
    auto wtitle = internal::str2wstr(title);
    // using set context to apply new visual style (required for all windows
    // versions)
    new_style_context ctx;
    *exit_code =
